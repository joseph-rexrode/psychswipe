	...
	<div class="wrapper matches">
		...
		<div class="main rows">
			<c:forEach items="${matches}" var="match">
				<div class="row">
					<c:choose>
						<c:when test="${match.getUser().getProfilePhoto() != null}">
							<a class="pfp" href="/patient/matches/${match.getId()}" style="background: center / cover no-repeat url(${match.getUser().getProfilePhotoImagePath()})"></a>
						</c:when>
						<c:otherwise>
							<a class="pfp" href="/patient/matches/${match.getId()}"></a>
						</c:otherwise>
					</c:choose>
				...
				<div class="row--moreInfo hidden">
					<div>
						...
						<p>Insurance Accepted: 
							<c:forEach items="${match.getInsuranceProvidersAccepted()}" var="insProvider">
								${insProvider} | 
							</c:forEach>
						</p>
						<p>States Licensed In: 
							<c:forEach items="${match.getStatesLicensedIn()}" var="state">
								${state} | 
							</c:forEach>
						</p>
					</div>
